...
$functions_path = get_template_directory_uri() . '/';
...
function siteoptions_add_admin() {
    ...
    $of_page = add_theme_page('主题设置', '主题设置', 'edit_theme_options', 'siteoptions','siteoptions_options_page');
...
<div class="wrap" id="truethemes_container">
  <div id="of-popup-save" class="of-save-popup">
    <div class="of-save-save">保存</div>
  </div>
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset">重置</div>
  </div>
  <form action="" enctype="multipart/form-data" id="ofform">
    ...
    <img style="display:none;" src="<?php echo get_template_directory_uri() ?>/geeku_framework/admin/images/wpspin_light.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
  ...
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    ...
    <input name="reset" type="submit" value="重置" class="button submit-button reset-button" onclick="return confirm('提示: 所有的设置项都会重置。确定吗？');" />
    <input type="hidden" name="of_save" value="重置" />
    </span>
  </form>
</div>
...
function of_style_only() {
	wp_enqueue_style('admin-style',   get_template_directory_uri()  . '/geeku_framework/admin/admin-style.css');
	wp_enqueue_style('color-picker',  get_template_directory_uri()  . '/geeku_framework/admin/colorpicker.css');
	$color = get_user_option('admin_color');
	if ($color == "fresh")
		{
		wp_enqueue_style('admin-style-grey', get_template_directory_uri()  . '/geeku_framework/admin/admin-style-grey.css');
		wp_enqueue_style('color-picker', get_template_directory_uri()  . '/geeku_framework/admin/colorpicker.css');
		}
}
...
function of_load_only() {
	...
	wp_register_script('jquery-input-mask',  get_template_directory_uri()  . '/geeku_framework/admin/js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('jquery-input-mask');
	wp_enqueue_script('color-picker',  get_template_directory_uri()  . '/geeku_framework/admin/js/colorpicker.js', array('jquery'));
	wp_enqueue_script('ajaxupload',  get_template_directory_uri()  . '/geeku_framework/admin/js/ajaxupload.js', array('jquery'));
...
function siteoptions_uploader_function($id,$std,$mod){
	...
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">上传</span>';
	
	if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
	
	$uploader .= '<span class="button image_reset_button '. $hide.'" id="reset_'. $id .'" title="' . $id . '">清除</span>';
	$uploader .='</div>' . "\n";
